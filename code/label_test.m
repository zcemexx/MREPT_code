...
% 输入数据：仍然放在 projectRoot/data/3T_Brain_phantom
...
% 输出数据：统一写到 ~/Scratch/3T_Brain_phantom/phase2_results/...
homeDir    = getenv('HOME');                     % e.g. /myriadfs/home/zcemexx
scratchDir = fullfile(homeDir, 'Scratch');       % ~/Scratch
phase2Root = fullfile(scratchDir, datasetName, 'phase2_results');
...
phiPath = fullfile(baseDir, 'Transceive_phase_noiseless.nii.gz');
fprintf('DEBUG: baseDir = %s\n', baseDir);
fprintf('DEBUG: phiPath = %s\n', phiPath);
fprintf('DEBUG: exist(phiPath,''file'') = %d\n', exist(phiPath, 'file'));

phi0_nii = nii_tool('load', phiPath);
...
if exist(segPath, 'file')
    ...
    % ----- define conductivity ranges (adjust if needed) -----
    wm_mask  = sigma > 0.20 & sigma < 0.50;
    gm_mask  = sigma >= 0.50 & sigma < 1.00;
    csf_mask = sigma >= 1.00;
...
radius_list = 1:20;        % radii to sweep (for differential family)
